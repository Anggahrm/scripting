...
local function goToWorld(name)
    sendNotification("🌍 Traveling to " .. name)
...
local function fRealtimePlayerWatcher()
    ...
    if count ~= lastPlayerCount then
        if count > 0 then
            ...
            sendNotification("✅ No players nearby.")
        end
...
local function eatAllGems()
    ...
    sendNotification("💰 Eating " .. count .. " gems...")
    for i = 1, count do
        put(config.gemID, 0, 0)
        stats.gemsEaten = stats.gemsEaten + 1
        Sleep(100)
    end
    sendNotification("✅ All gems eaten!")
end

local function lumberTrees()
    while stats.isRunning do
        if config.detectPlayer and playerDetected then
            Sleep(2000)
        else
            local axeCount = getItemCount(config.axeID)
            if axeCount < config.minAxe then
                sendNotification("⚠️ No axe left!")
                return
            end
            ...
            if #treeTiles == 0 then
                sendNotification("🔍 No trees found, waiting...")
                Sleep(1000)
            else
                ...
                for _, t in ipairs(ordered) do
                    ...
                    if findPath(t.x, t.y) then
                        local tries = 0
                        while tries < 30 do
                            local px = math.floor(getLocal().posX / 32)
                            local py = math.floor(getLocal().posY / 32)
                            if px == t.x and py == t.y then break end
                            Sleep(100)
                            tries = tries + 1
                        end
                        local dx = t.x - math.floor(getLocal().posX / 32)
                        local dy = t.y - math.floor(getLocal().posY / 32)
                        hit(dx, dy)
                        stats.treesCleared = stats.treesCleared + 1
                        if stats.treesCleared % 10 == 0 then
                            sendNotification("🌳 Trees cleared: " .. stats.treesCleared)
                        end
...
local function mainLoop()
    ...
    sendNotification("▶️ Auto Lumberjack Started!")
    while stats.isRunning do
        local axeCount = getItemCount(config.axeID)
